                    ...
                    <div class="table-main table-responsive">
                        <table class="table">
                            <thead>
								...
								<c:if test="${sessionScope.sid == '관리자'}">
									<th>답글</th>
								</c:if>
                            ...
                            <c:forEach items="${QnAlist}" var="list">
								...
								<c:if test="${list.seq == 1}">		
                                    <td class="name-pr">
                                        <a href="<%=request.getContextPath()%>/QnAServlet?command=QnA_Question_Detail&selNo=${list.no}">
											${list.title }
										</a>
                                    </td>
								</c:if>			
                                ...
                                <c:if test="${sessionScope.sid == '관리자'}">
									<th>
										<a href="<%=request.getContextPath()%>/QnAServlet?command=QnA_Answer&answerNo=${list.no }">답변하기</a>
									</th>
								</c:if>
         ...
         -->       
         	...
         	<c:if test="${sessionScope.sid != '관리자'}">
				<th>
					<c:choose>
						<c:when test="${sessionScope.sid != null}">
             				<div class="col-12 d-flex shopping-box"><a href="<%=request.getContextPath()%>/freshshop-master/QnAquestion.jsp" class="ml-auto btn hvr-hover">글 쓰기</a> </div>
						</c:when>
					<c:otherwise>
						<c:if test="${sessionScope.sid == null}">
							<div class="col-12 d-flex shopping-box"><a href="<%=request.getContextPath()%>/login/login.jsp" class="ml-auto btn hvr-hover">글 쓰기</a> </div>
						</c:if>
					</c:otherwise>
					</c:choose>
				</th>
