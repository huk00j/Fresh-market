...
function selectDelRow() {
	alert("?");
	var chk = document.getElementsByName("selNo"); // 체크박스객체를 담는다
...
for (var i = 1; i < $('selNo').size(); i++) {
	// table 중 tr이 i번째 있는 자식중에 체크박스가 체크중이면
	
	var chk = $('selNo').eq(i).children().find('input[type="checkbox"]').is(':checked');

	if (chk == true) {
	// 그 i 번째 input text의 값을 가져온다.
		var abc = $('selNo').eq(i).find('input[type="number"]').val();
	...
	logininfo.action = "<%=request.getContextPath()%>/GuestServlet?command=cart_update123";
	logininfo.submit();
 ...
 function mySubmit(index){
    if(index==1) {
       document.update_cart.action='<%=request.getContextPath()%>/GuestServlet?command=cart_update'; 
    }
    
    if(index==2) {
    	document.update_cart.action='<%=request.getContextPath()%>/GuestServlet?command=cart_buy'; 
    }
...
<body>
    ...
    <div class="cart-box-main">
        ...
        <form class="update_cart" name="update_cart" method="post">
      
        <%-- from 만들 때 클래스명 꼭 선언 해줘야한다!!!!!!! --%>
                    ...
                    <div class="table-main table-responsive">
                        <table class="table">
                            ...
                            <c:forEach items="${vlist}" var="list">
                            	<input type="hidden" name="cartNo" value="${list.no }">
                                <tr>
                            		...
                            		<td>
                            			...
                            			--%>
                            				<input type="checkbox" id="same-address" checked="${list.no }" name="selectNo" value="${list.no }">
                            			</div>
                            		</td>
                            	 
                                    <td class="thumbnail-img">
                                        <a href="<%=request.getContextPath()%>/freshshop-master/#">
									<img class="img-fluid" src="<./upload/${list.image }" alt="" />
								</a>
                                    </td>
                                    <td class="name-pr">
                                        <a href="<%=request.getContextPath()%>/freshshop-master/#">
                                    ...
                                    <td class="quantity-box"><input type="number" size="4" name="cartAmount" value="${list.amount }" min="1" step="1" class="c-input-text qty text"></td>
                                    <td class="total-pr">
                                        <p>$ ${list.amount * list.price }</p>
                                    </td>
                                    
                                 <!-- 변수 선언 만들어봄
                                    <c:set var="totalAmount" value="${list.amount * list.price }"></c:set>
                                  -->   
<%--                                    <td class="add-pr">
                                    	<a class="btn hvr-hover" data-fancybox-close=""
										href="<%=request.getContextPath()%>/GuestServlet?command=cart_update&cartNo=${list.no }&updateAmount=${cartAmount }">수량 변경</a>
									</td>
    --%>                              <!--    	
                                    	href="javascript:document.update_cart.submit();">수량 변경</a>
								-->
										
										
										 <td class="add-pr"><input type="button" class="btn hvr-hover"
                               onClick="mySubmit(1);" value="수량 변경">
                              </td>
										
										
                                    
                                    <td class="remove-pr">
                                        <a href="<%=request.getContextPath()%>/GuestServlet?command=cart_delete&deleteNo=${list.no }&deleteId=${sessionScope.sid}">
       ...
       <!--         
                <div class="col-lg-6 col-sm-6">
                    <div class="update-box">
                        <input value="장바구니 변경" type="submit">
                    </div>
                </div>
         -->
           ...
           <%--
                <div class="col-12 d-flex shopping-box"><a href="<%=request.getContextPath()%>/freshshop-master/checkout.html" class="ml-auto btn hvr-hover">주문하기</a> </div>
           --%>
           		
           		 <div class="col-12 d-flex shopping-box">
           		 <input type="button" class="ml-auto btn hvr-hover"
                               onClick="mySubmit(2);" value="주문하기"> </div>
